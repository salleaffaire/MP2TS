...
class Demux : public BinaryInFile {
 public:
      ...
      if ((mSize - mCurrentPointer) >= TS_PACKET_LENGTH) {
        if (mData[mCurrentPointer++] == 0x47) {
          ...
          if (mAdaptationFieldExist) {
            ...
            // Adaptation Field Length is the number of bytes in the AF
            // following the Adaptation Field Length field
            ...
            if (mTransportPrivateDataFlag) {
              unsigned int transportPrivateDataLength =
                  mData[mCurrentPointer++];
              for (int i = 0; i < transportPrivateDataLength; i++) {
                mAdaptationFieldPrivateData[i] = mData[mCurrentPointer++];
              }
              if (mVerbose) {
            ...
            uint32_t number_of_stuffing_bytes =
                mAdaptationFieldLength - (mCurrentPointer - starting_pointer);
  ...
  bool ParseAssigned(PES_Packet *packet) {
    ...
    // If PAT is not yet parsed, look into the
    if (!mIsPATParsed) {
      // Do we have a PAT?
      if (AsAPIDPacket(0)) {
        // Parse PAT
    ...
    if (IsAPMTPID(packet->mPID)) {
      ...
      if (start_code == 0x000001) {
        uint8_t stream_id = *data;
        data++;

        if (mVerbose) {
          std::cout << "  Stream ID : 0x" << std::hex << std::setfill('0')
                    << std::setw(2) << (int)stream_id << std::dec << std::endl;
        }
        ...
        if (packet->mPTSDTSIndicator & 0x2) {
          uint8_t high = *data;
          data++;
          uint16_t mid = (*data << 8) + (*(data + 1));
          data += 2;
          uint16_t low = (*data << 8) + (*(data + 1));
          data += 2;
